...
from context import (
    ...
    GetEntityAction,
    GetEntityChangesAction,
    GetEntityRolesAction,
    GetEntityAddressesAction,
    GetEntityTradingNamesAction,
    GetEntityPhoneNumbersAction,
    GetEntityEmailAddressesAction,
    GetEntityWebsitesAction,
    GetEntityGstNumbersAction,
    GetEntityIndustryClassificationsAction,
    GetEntityCompanyDetailsAction,
)
...
async def test_search_entities_success(mock_context):
    ...
    assert result.data["items"][0]["nzbn"] == "9429000106078"
    assert result.data["items"][0]["entity_name"] == "Test Company Ltd"
...
async def test_get_entity_success(mock_context):
    ...
    assert result.data["nzbn"] == "9429000106078"
    assert result.data["entity_name"] == "Test Company Ltd"
    assert len(result.data["addresses"]) == 1
    assert len(result.data["roles"]) == 1
    assert result.data["roles"][0]["person_name"] == "John Smith"
...
async def test_get_entity_roles_success(mock_context):
    ...
    assert result.data["roles"][0]["person_name"] == "Jane Doe"
    assert result.data["roles"][1]["entity_name"] == "Holding Co"
...
async def test_get_entity_addresses_success(mock_context):
    ...
    assert result.data["nzbn"] == "9429000106078"
    assert len(result.data["addresses"]) == 2
    assert result.data["addresses"][0]["address_type"] == "RegisteredOffice"
...
async def test_get_entity_trading_names_success(mock_context):
    ...
    assert len(result.data["trading_names"]) == 2
    assert result.data["trading_names"][0]["name"] == "Trading Name 1"
...
async def test_get_entity_phone_numbers_success(mock_context):
    ...
    assert len(result.data["phone_numbers"]) == 1
    assert result.data["phone_numbers"][0]["phone_number"] == "1234567"
...
async def test_get_entity_email_addresses_success(mock_context):
    ...
    assert len(result.data["email_addresses"]) == 1
    assert result.data["email_addresses"][0]["email"] == "info@test.co.nz"
...
async def test_get_entity_websites_success(mock_context):
    ...
    assert len(result.data["websites"]) == 1
    assert result.data["websites"][0]["url"] == "https://www.test.co.nz"
...
async def test_get_entity_gst_numbers_success(mock_context):
    ...
    assert len(result.data["gst_numbers"]) == 1
    assert result.data["gst_numbers"][0]["gst_number"] == "123-456-789"
...
async def test_get_entity_industry_classifications_success(mock_context):
    ...
    assert result.data["classifications"][0]["code"] == "L672000"
...
async def test_get_entity_company_details_success(mock_context):
    ...
    assert result.data["company_details"]["constitution_filed"] is True
    assert result.data["company_details"]["annual_return_filing_month"] == 3
