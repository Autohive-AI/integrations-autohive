"""
Replicate the exact agent input to test font sizing issues.
Uses HK Template.pptx as the base file.
"""
...
async def test_agent_input():
    """Replicate agent's exact find_and_replace input"""

    print("=" * 70)
    print("AGENT INPUT REPLICATION TEST")
    print("=" * 70)
    print("\nReplicating exact agent input to test font sizing...")
    ...
    async with ExecutionContext(auth=auth) as context:
        ...
        # Get initial state
        print("\n3. Getting initial slide elements...")
        initial_state = await slide_maker.execute_action(
            "get_slide_elements",
            {
        ...
        print(f"   [OK] Found {initial_state['total_slides']} slides")
        print(f"   [OK] Total elements: {initial_state['total_elements']}")

        # Execute the exact find_and_replace operation from agent
        print("\n4. Executing find_and_replace (15 replacements)...")
        replacements = [
        ...
        print(f"   [OK] Total replacements made: {result['total_replacements']}")
        print(f"   [OK] Processed: {result['processed']}")
        if result['blocked']:
            print(f"   [WARN] Blocked operations: {len(result['blocked'])}")
        if result['warnings']:
            print(f"   [WARN] Warnings: {len(result['warnings'])}")

        files = [result["file"]]

        # Get final state with content
        ...
        output_path = os.path.join(
            ...
            f"test_agent_replication_{int(time.time())}.pptx"
        )
        ...
        print("\n" + "=" * 70)
        print("[SUCCESS] AGENT INPUT REPLICATED!")
        print("=" * 70)
        print("\nPLEASE OPEN THE FILE AND CHECK:")
        print("  1. Which text boxes have fonts that are TOO BIG")
        print("  2. Which text boxes have fonts that are TOO SMALL")
        print("  3. Compare with expected sizing")
        print("\nThis will help identify which sizing scenarios need adjustment.")
        print(f"\n[FILE] Output: {output_path}")
        print("=" * 70)


if __name__ == "__main__":
    asyncio.run(test_agent_input())
