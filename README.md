...
[ahrefs](ahrefs): SEO analytics integration with Ahrefs API v3 for backlink analysis, domain ratings, keyword research, and organic traffic insights. Supports domain rating retrieval (DR and Ahrefs Rank), comprehensive backlink analysis (stats, full backlink lists, referring domains), organic search data (keywords a domain ranks for, top pages by traffic), keyword metrics (volume, difficulty, CPC for up to 10 keywords), and outgoing link analysis. Features custom authentication with API key (Enterprise plan required), flexible analysis modes (exact URL, domain, subdomains, prefix), sorting and pagination options, and country-specific search data. Includes 8 actions covering domain analysis (1), backlinks (3), organic search (2), keywords (1), and link analysis (1). Ideal for SEO research, competitor analysis, link building, and content strategy workflows.
