...
[nzbn](nzbn): Comprehensive integration with the New Zealand Business Number (NZBN) API for searching and retrieving business entity information from the NZBN Register. Supports searching entities by name, trading name, NZBN, or company number with filtering by entity status, type, and industry code. Provides full entity details including addresses, roles (directors, shareholders, partners), trading names, phone numbers, email addresses, websites, GST numbers, industry classifications, company details, trading areas, and change history. Features entity change tracking for monitoring business updates within specified time periods. Includes 14 actions covering search, entity details, roles, addresses, contacts, classifications, company details, history, and trading areas. Uses 2-legged OAuth authentication with Microsoft B2C token endpoint and supports both sandbox and production environments. Ideal for business verification, due diligence, company research, and regulatory compliance workflows.
